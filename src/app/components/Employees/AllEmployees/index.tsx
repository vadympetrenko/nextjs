...
import { EmployeeTypeStoreExtended } from "@/models/Employee";
import React, { useState } from "react";
import classes from "@/app/components/Employees/AllEmployees/styles.module.scss";
...
export const AllEmployees: React.FC<AllEmployeesType> = ({
  ...
  const [allEmployees, setEmployess] =
    useState<EmployeeTypeStoreExtended[]>(employees);
    ...
    <>
      <Toolbar>
        <Link href={"/admin/employees/add-employee"}>
          <Button variation="primary" className={classes.AddEmployee}>
            <TiUserAddOutline />
            Add Employee
          </Button>
        </Link>
            ...
            {allEmployees.map((employee) => (
              <React.Fragment key={employee.phone_number}>
                ...
                >
                  ...
                  <td>
                    {employee?.store?.length && Array.isArray(employee.store)
                      ? employee?.store?.map((item, index, arr) =>
                  ...
                  <td className={classes.actions}>
                    <TiPencil onClick={editHandler.bind(null, employee._id!)} />
                    <TiTrash onClick={toggleModalHandler} />
                  </td>
                </tr>
                <Modal
                  ...
                  primaryAction={{
                    ...
                    action: removeEmployeeHandler.bind(null, employee._id!),
                  }}
                >
